...
public class SimpleContext implements Context, Pipeline, Lifecycle {
    ...
    public synchronized void start() throws LifecycleException {
        ...
        started = true;
        try {
            // 启动子类的lifeCycle的start
            if ((loader != null) && (loader instanceof Lifecycle)) {
                ((Lifecycle) loader).start();
            }

            Container[] childrens = findChildren();
            for (int i = 0; i < childrens.length; i++) {
                if (childrens[i] instanceof Lifecycle) {
                    ((Lifecycle) childrens[i]).start();
                }
            }
            // start valve in pipeline include basic valve
    ...
    public Container[] findChildren() {
        synchronized (children) {
            ...
            return (Container[]) children.values().toArray(result);
        }
    }
